...
map = new mapboxgl.Map({
  container: "map",
  style: "mapbox://styles/nae/clattlhyy000916pi3ju078vi",
...
var svg = d3.select("#menuSVG").append("svg");
...
var stateData = d3.csv("acs_data/stateData_2023_final.csv");
...
var d3projection = d3
  .geoAlbersUsa()
  .translate([bboxPro.width / 2, bboxPro.height / 2])
  .scale(scale);
var projectionAl = d3
  .geoAlbersUsa()
  .translate([bboxPro.width / 2, bboxPro.height / 2])
  .scale(scale);
var projectionMercartor = d3
  .geoMercator()
  .translate([bboxPro.width / 2, bboxPro.height / 2])
  .scale(scale);
...
Promise.all([stateData, countyData, districtData, msaData, centers]).then(
  initNation
);
...
layerShape
  ...
  .on("mouseover", function (d) {
    layerShape
      .selectAll(".districtDiamond")
      .transition()
      .duration(100)
      .attr("d", diamond.size(2000))
      .style("stroke-width", 3);
  ...
  .on("click", function (d) {
    ...
    layerShape
      ...
      .style("stroke-width", 3);
    layerShape
      .selectAll(".nationDiamond")
      .transition()
      .duration(100)
      .attr("d", diamond.size(1250))
      .style("stroke-width", 3);
    layerShape
      .selectAll(".countyDiamond")
      .transition()
      .duration(100)
      .attr("d", diamond.size(1250))
      .style("stroke-width", 3);
    layerShape
      .selectAll(".msaDiamond")
      .transition()
      .duration(100)
      .attr("d", diamond.size(1250))
      .style("stroke-width", 3);
    layerShape
      .selectAll(".stateDiamond")
      .transition()
      .duration(100)
      .attr("d", diamond.size(1250))
      .style("stroke-width", 3);
...
function nationStart() {
  ...
  cssChange.innerHTML =
    "<style> .sidebar-content { background-color: #f7f7f7;} " +
...
function stateStart() {
  ...
  cssChange.innerHTML =
    "<style> .sidebar-content { background-color: #393745;} " +
    ...
    "#nationSource {color: #fff;}" +
    "a.nationLink {color: #fff !important;}" +
    "a.titleLink {color: #ffffff !important;}" +
...
function districtStart() {
  ...
  cssChange.innerHTML =
    "<style> .sidebar-content { background-color: #000;} " +
...
function countyStart() {
  ...
  cssChange.innerHTML =
    "<style> .sidebar-content { background-color: #87d9f7;} " +
    ".sidebar-share {background: #87d9f7}" +
    ".intro-text {color: #000;} " +
...
function msaStart() {
  msaMap(msaDataLoad);
  cssChange.innerHTML =
    "<style> .sidebar-content { background-color: #6163ed;} " +
    ".intro-text {color: #fff;} " +
    ".site-title {color: #fff;} " +
    ".sidebar-share {background: #6163ed}" +
    ".mti_tooltip_text_cont {background-color: #6163ed;} " +
    ".grid_stat_descr.stat1 {background-color: #6a7ef5;}" +
    ".grid_stat.stat1 {background-color: #6a7ef5;} " +
    ".mti_tooltip_hed {color: #fff;} " +
    ".grid_stat_descr {color: #fff;} " +
    ".grid_stat {color: #fff;} " +
    ".see_more_button {color: #fff !important;}" +
    ".source-line {color: #fff;}" +
    "#home {background-color: #6163ed;}" +
    "#nationSource {color: #fff;}" +
    "a.nationLink {color: #fff !important;}" +
    "a.titleLink {color: #fff !important;}" +
  ...
  map.on("load", function () {
    ...
    map.on("click", "state-fills", function (e) {
      ...
      sidebarSummary.innerHTML =
        ...
        numberWithCommas(state_feat.fbpop) +
        "<br>‍</div>" +
        "<div class='grid_stat_descr'>Immigrant Share of Population:</div>" +
        "<div class='grid_stat'>" +
        state_feat.fbshare +
        ...
        "<div class='grid_stat_descr stat1'>Immigrant Taxes Paid:</div>" +
        "<div class='grid_stat stat1'>" +
        dollars(state_feat.tottax) +
        "</div>" +
        "<div class='grid_stat_descr'>Immigrant Spending Power:</div>" +
        "<div class='grid_stat'>" +
        dollars(state_feat.sppower) +
        "</div>" +
        "<div class='grid_stat_descr stat1'>Number of Immigrant Entrepreneurs:</div>" +
        "<div class='grid_stat stat1' class='vertalign'>" +
        numberWithCommas(state_feat.entrp) +
        "</div>" +
        "<div class='grid_stat_descr'>Number of Eligible Immigrant Voters:</div>" +
        "<div class='grid_stat'>" +
...
function countyMap(data) {
  ...
  map = new mapboxgl.Map({
    container: "map",
    style: "mapbox://styles/nae/clatuta50000s15pub1rqpq73",
    zoom: 2.65 * Math.log10(window.innerWidth / 50),
    center: [9.5, 0],
    maxZoom: 9,
    attributionControl: true,
  ...
  map.on("load", function () {
    ...
    map.on("click", "county-fills-click", function (e) {
      ...
      if (map.getLayer("county-search-fill"))
        map.removeLayer("county-search-fill");
      //
    ...
    map.on("click", "county-fills", function (e) {
      ...
      if (county_feat.noData != 1.0) {
        sidebarSummary.innerHTML =
          ...
          "<h1 class='mti_tooltip_hed'>" +
          county_feat.countyName +
          "</h1>" +
          "<div class='w-layout-grid grid_data'>" +
          "<div class='grid_stat_descr stat1'>Number of Immigrants</div>" +
          "<div class='grid_stat stat1'>" +
          numberWithCommas(county_feat.immigrantPop) +
          "<br>‍</div>" +
          "<div class='grid_stat_descr'>Immigrant Share of Population</div>" +
          "<div class='grid_stat'>" +
          county_feat.fbShare +
          ...
          "<div class='grid_stat_descr stat1'>Immigrant Taxes Paid</div>" +
          "<div class='grid_stat stat1'>" +
          dollars(county_feat.immTaxes) +
          "</div>" +
          "<div class='grid_stat_descr'>Immigrant Spending Power</div>" +
          "<div class='grid_stat'>" +
          dollars(county_feat.immSpend) +
          "</div>" +
          "<div class='grid_stat_descr stat1' >Number of Immigrant Entrepreneurs</div>" +
          "<div class='grid_stat stat1' class='vertalign'>" +
          numberWithCommas(county_feat.immEntre) +
          "</div>" +
          "<div class='grid_stat_descr'>Number of Eligible Immigrant Voters</div>" +
          "<div class='grid_stat'>" +
          numberWithCommas(county_feat.immVote) +
          "</div></div>" +
        ...
        sidebarSummary.innerHTML =
          ...
          "/'>" +
          "More " +
          county_feat.stateName +
          " Data" +
          "<i class='fa fa-long-arrow-right fa-adjust'></i></a></div></div>";
      }
    });
  });
  ...
  map.on("load", function () {
    ...
    map.on("click", "district-fills", function (e) {
      ...
      sidebarSummary.innerHTML =
        ...
        "<div class='grid_stat_descr stat1'>Number of Immigrants:</div>" +
        "<div class='grid_stat stat1'>" +
        numberWithCommas(district_feat.immigrantPop) +
        "<br>‍</div>" +
        "<div class='grid_stat_descr'>Immigrant Share of Population:</div>" +
        "<div class='grid_stat'>" +
        district_feat.immigrantShare +
        "</div>" +
        "<div class='grid_stat_descr stat1'>Immigrant Taxes Paid:</div>" +
        "<div class='grid_stat stat1'>" +
        dollarsCD(district_feat.immCDTaxes) +
        "</div>" +
        "<div class='grid_stat_descr'>Immigrant Spending Power:</div>" +
        "<div class='grid_stat'>" +
        dollarsCD(district_feat.immCDSpend) +
        "</div>" +
        "<div class='grid_stat_descr stat1'>Number of Immigrant Entrepreneurs:</div>" +
        "<div class='grid_stat stat1' class='vertalign'>" +
        numberWithCommas(district_feat.immCDEntr) +
        "</div>" +
        "<div class='grid_stat_descr'>Number of Eligible Immigrant Voters:</div>" +
        "<div class='grid_stat'>" +
    ...
    map.on("click", "msa-fills", function (e) {
      ...
      if (msa_feat.immigrantMSAPop != undefined) {
        sidebarSummary.innerHTML =
          ...
          "<div class='grid_stat_descr stat1'>Number of Immigrants:</div>" +
          "<div class='grid_stat stat1'>" +
          numberWithCommas(msa_feat.immigrantMSAPop) +
          "<br>‍</div>" +
          "<div class='grid_stat_descr'>Immigrant Share of Population:</div>" +
          "<div class='grid_stat'>" +
          msa_feat.immigrantMSAShare +
          "</div>" +
          "<div class='grid_stat_descr stat1'>Immigrant Taxes Paid:</div>" +
          "<div class='grid_stat stat1'>" +
          dollars(msa_feat.immiMSATaxes) +
          "</div>" +
          "<div class='grid_stat_descr'>Immigrant Spending Power:</div>" +
          "<div class='grid_stat'>" +
          dollars(msa_feat.immiMSASpend) +
          "</div>" +
          "<div class='grid_stat_descr stat1'>Number of Immigrant Entrepreneurs:</div>" +
          "<div class='grid_stat stat1' class='vertalign'>" +
          numberWithCommas(msa_feat.immiMSAEntre) +
          "</div>" +
          "<div class='grid_stat_descr'>Number of Eligible Immigrant Voters:</div>" +
          "<div class='grid_stat'>" +
...
function forwardGeocoder(query) {
  ...
  if (activeMap == "CountyView") {
    for (var i = 0; i < countyList.length; i++) {
      ...
      if (
        featureRegion.county_name_census
          .toLowerCase()
          .search(query.toLowerCase()) !== -1
      ) {
        featureRegion["place_name"] = featureRegion.county_name_census;
        featureRegion["center"] = [
          featureRegion.Longitude,
          featureRegion.Latitude,
        ];
  ...
  if (activeMap == "DistrictView") {
    for (var i = 0; i < districtList.length; i++) {
      ...
      if (
        featureRegion.DistrictName.toLowerCase().search(query.toLowerCase()) !==
        -1
      ) {
        featureRegion["place_name"] = featureRegion.DistrictName;
        featureRegion["center"] = [
          featureRegion.INTPTLON,
          featureRegion.INTPTLAT,
        ];
...
function countyGetLatLon(e) {
  ...
  if (displayFeat.state.noData != 1.0) {
    sidebarSummary.innerHTML =
      ...
      // "<h1 class='mti_tooltip_hed'>" + county_prop.NAMELSAD + ", " + county_prop.stateabbr + "</h1>" +
      "<h1 class='mti_tooltip_hed'>" +
      displayFeat.state.countyName +
      "</h1>" +
      "<div class='w-layout-grid grid_data'>" +
      "<div class='grid_stat_descr stat1'>Number of Immigrants</div>" +
      "<div class='grid_stat stat1'>" +
      numberWithCommas(displayFeat.state.immigrantPop) +
      "<br>‍</div>" +
      "<div class='grid_stat_descr'>Immigrant Share of Population</div>" +
      "<div class='grid_stat'>" +
      displayFeat.state.fbShare +
      "</div>" +
      "<div class='grid_stat_descr stat1'>Immigrant Taxes Paid</div>" +
      "<div class='grid_stat stat1'>" +
      dollars(displayFeat.state.immTaxes) +
      "</div>" +
      "<div class='grid_stat_descr'>Immigrant Spending Power</div>" +
      "<div class='grid_stat'>" +
      dollars(displayFeat.state.immSpend) +
      "</div>" +
      "<div class='grid_stat_descr stat1' >Number of Immigrant Entrepreneurs</div>" +
      "<div class='grid_stat stat1' class='vertalign'>" +
      numberWithCommas(displayFeat.state.immEntre) +
      "</div>" +
      "<div class='grid_stat_descr'>Number of Eligible Immigrant Voters</div>" +
      "<div class='grid_stat'>" +
      numberWithCommas(displayFeat.state.immVote) +
      "</div></div>" +
      "<div class='source-line'>" +
      "Source: " +
      displayFeat.state.source +
      "</div>" +
      "<br>‍</div>";
  } else {
    sidebarSummary.innerHTML =
...
function stateGetLatLon(e) {
  ...
  map.setFeatureState({ source: "state", id: clickedStateId }, { click: true });
...
function msaGetLatLon(e) {
  ...
  map.setFeatureState({ source: "msa", id: clickedStateId }, { click: true });
